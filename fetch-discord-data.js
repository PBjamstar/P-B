...
/**
 * Reads and parses a response body as JSON, retrying the whole request
 * (via refetch callback) if the body stream closes prematurely.
 */
async function readJsonWithRetry(refetchFn, maxAttempts = 4) {
  for (let attempt = 1; attempt <= maxAttempts; attempt++) {
    try {
      ...
      const transient = err.code === 'ERR_STREAM_PREMATURE_CLOSE'
        || err.code === 'ECONNRESET'
        || /premature close/i.test(err.message || '');
      if (transient && attempt < maxAttempts) {
        console.warn(`⚠️  Request failed (attempt ${attempt}/${maxAttempts}): ${err.message}. Retrying...`);
...
async function fetchAllMembers() {
  ...
  while (true) {
    const batch = await readJsonWithRetry(() => fetch(
      `https://discord.com/api/v10/guilds/${GUILD_ID}/members?limit=1000&after=${after}`,
      { headers: { Authorization: `Bot ${TOKEN}`, 'Accept-Encoding': 'identity' } }
    ));
...
async function fetchGuild() {
  return readJsonWithRetry(() => fetch(
...
async function main() {
  ...
  try {
    ...
    while (true) {
      ...
      const bRes = await fetch(url, { headers: { Authorization: `Bot ${TOKEN}`, 'Accept-Encoding': 'identity' } });
      if (!bRes.ok) break;
