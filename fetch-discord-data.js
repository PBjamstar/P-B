...
async function exportDonations(allMembers) {
  ...
  const statusData = {
    updatedAt: new Date().toISOString(),
    memberCount: roster.totalMembers,
    trialCount: roster.trialCount,
    totalServerMembers: guild.approximate_member_count || 0,
    lastRosterUpdate: new Date().toISOString(),
    blacklistCount: JSON.parse(fs.readFileSync('data/blacklist.json', 'utf-8')).length,
    formerMemberCount: JSON.parse(fs.readFileSync('data/former-members.json', 'utf-8')).length,
  };
