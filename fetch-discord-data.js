...
async function exportDonations() {
  ...
  try {
    ...
    // Find the pinned message tagged ||STORE:donations||
    const donationPin = pins.find(m => m.content.startsWith('||STORE:donations||'));
    if (!donationPin) {
      console.warn('⚠️ No donations pin found');
      fs.writeFileSync('data/donations.json', JSON.stringify({ goal: 0, goalLabel: '', donations: [] }, null, 2));
      return;
    }
    // Parse JSON from the pin content (line after the tag)
    const lines = donationPin.content.split('\n');
    lines.shift(); // remove tag line
    const data = JSON.parse(lines.join('\n'));
    // Write a trimmed version for the website (no sensitive IDs)
    const webData = {
      goal: data.goal || 0,
    ...
    console.log(`✅ Donations saved: ${webData.donations.length} entries, goal: ${webData.goal}`);
  } catch (err) {
