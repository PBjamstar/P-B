      ...
      for (const msg of msgs) {
        ...
        if (idField) {
          ...
          if (idMatch) {
            ...
            // Fetch the actual username from Discord API
            let discordUsername = '';
            try {
              const uRes = await fetch(`https://discord.com/api/v10/users/${userId}`, {
                headers: { Authorization: `Bot ${TOKEN}` }
              });
              if (uRes.ok) {
                const uData = await uRes.json();
                discordUsername = (uData.username || uData.global_name || '').toLowerCase();
              }
            } catch {}
            // Strip emojis and normalise IGN for reliable matching
            const rawIgn = ignField?.value || '';
            const ign = rawIgn
              .replace(/[\u{1F000}-\u{1FFFF}]|[\u{2600}-\u{27BF}]|[\u{1F300}-\u{1F9FF}]/gu, '')
              .replace(/\[.*?\]/g, '') // strip [tags] e.g. [P-B]
              .replace(/,\s*$/, '')     // trailing comma
              .trim()
              .toLowerCase();
            blacklisted.push({ userId, discordUsername, ign });
          }
        }
      }
