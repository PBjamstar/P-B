    ...
    while (true) {
      ...
      for (const msg of msgs) {
        ...
        if (idField) {
          ...
          // Extract discord username — value format: "<@ID> (ID: 123...)" or "username (ID: 123...)"
          const usernameMatch = idField.value.match(/<@\d+>\s*\(ID:\s*\d+\)/)
            ? null
            : idField.value.match(/^([^(<]+)/);
          const discordUsername = idField.value
            .replace(/<@!?\d+>/, '')   // strip mention
            .replace(/\(ID:\s*\d+\)/, '') // strip ID part
            .trim()
            .toLowerCase() || '';
          if (idMatch) {
            blacklisted.push({
